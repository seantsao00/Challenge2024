...
def stage_defend_tower(api: API):
    ...
    if len(enemies_near_tower(find_tower(api),api)) > len(allies_near_tower(find_tower(api),api)) :#我方士兵大於敵軍，去蹲水晶
                ...
                for x in range(1, len(others_towers)) :
                    if calculate_distance(others_towers[x], find_fountain) != 0: 
                        if calculate_distance(min_dis , find_tower(api)) + calculate_distance(min_dis, find_fountain) >  calculate_distance(others_towers[x] , find_tower()) + calculate_distance(others_towers[x], find_fountain) :
                ...
                api.action_attack(api.get_owned_characters(), min_dis.position)
    ...
    if len(enemies_near_tower(find_tower(api),api)) > len(allies_near_tower(find_tower(api),api)):
