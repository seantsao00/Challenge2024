...
def stage_attack_tower(api: API):
    ...
    # 攻擊塔
    if info.target_tower is None or api.refresh_tower(info.target_tower) is None: return
    for ch in my_characters:
        if (api.refresh_character(ch).position - api.refresh_tower(info.target_tower).position).length() > 5:
            api.action_move_to(ch, api.refresh_tower(info.target_tower).position) # need target tower
...
def stage_defend_tower(api: API):
    ...
    if  len(my_melee)>= 15:
        ...
        for character in my_melee:
            if api.refresh_character(character) is not None and not api.get_movement(character).is_wandering:
                api.action_wander(api.refresh_character(character))
    defending_allies = detect_defending_allies(api)
 
 
    # move(api, api.get_owned_characters(), info.defend_tower.position)
    # 往我們要守的塔移動！
    for ch in defending_allies:
        info.defend_tower = api.refresh_tower(info.defend_tower)
        if api.refresh_character(ch) is not None and info.defend_tower is not None and (api.refresh_character(ch).position - info.defend_tower.position).length() > 5:
            api.action_move_to(api.refresh_character(ch), api.refresh_tower(info.defend_tower).position)
...
def detect_nearby_allies(api: API, point:Character):
    ...
    nearby_allies=[]
    dis_enemy = api.sort_by_distance(api.get_owned_characters(), api.refresh_character(point).position)
    for i in dis_enemy:
    # for i in api.get_visible_characters():
...
def every_tick(api: API):
    ...
    if info.stage == StageClass.EXPLORE:
        ...
        # 塔被人打下了，打回來！
        if info.defend_tower is not None and api.refresh_tower(info.defend_tower).team_id != api.get_team_id():
