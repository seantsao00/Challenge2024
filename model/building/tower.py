...
class Tower(LivingEntity):
    ...
    def __init__(self, position: pg.Vector2, team: Team, is_fountain: bool = False):
        ...
        self.character_type: const.CharacterType = const.CharacterType.RANGER
        ...
        self.attack_timer = Timer(int(1/self.attack_speed*1000), self.attack, once=False)
    ...
    def generate_character(self, timestamp=pg.time.get_ticks()):
        ...
        if self.character_type is const.CharacterType.MELEE:
            character_type = Melee
        elif self.character_type is const.CharacterType.RANGER:
            character_type = Ranger
        elif self.character_type is const.CharacterType.SNIPER:
            character_type = Sniper
        else:
            raise Exception(f'Character type Error: {self.character_type}')
        self.log.append((self.character_type, timestamp))
    ...
    def attack(self):
        ...
        for i in range(0, 4):
            if (self.team.party is const.PartyType.NEUTRAL or self.team.team_id != i) and self.enemy[i].front() != None and (victim == None or victim.time > self.enemy[i].front().time):
    ...
    def enemy_in_range(self, character: Character):
        if character.id in self.enemy[character.team.team_id].map or \
        ...
        self.enemy[character.team.team_id].push_back(character, get_model().get_time())

    def enemy_out_range(self, character: Character):
        if character.id not in self.enemy[character.team.team_id].map:
            return
        if character.position.distance_to(self.position) > self.attack_range or character.alive is False:
            ...
            self.enemy[character.team.team_id].delete(character)
