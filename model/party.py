...
class PartySelection:
    ...
    def handle_select_party(self, event: EventSelectParty):
        ...
        """
        if event.increase:
            self.selected_parties[event.index] = ((self.selected_parties[event.index] + 1)
                                                  % len(const.PartyType))
        else:
            self.selected_parties[event.index] = (self.selected_parties[event.index]
                                                  + len(const.PartyType) - 1) % len(const.PartyType)
