...
class Character(LivingEntity):
    ...
    def __set_wander_destination(self) -> bool:
        ...
        cnt = 0
        while (self.team.vision.position_inside_vision(destination) or get_model().map.get_position_type(destination) == const.MAP_OBSTACLE) and cnt < const.MAX_WANDERING:
            destination = pg.Vector2([uniform(0, const.ARENA_SIZE[0]),
                                     uniform(0, const.ARENA_SIZE[1])])
            cnt += 1
